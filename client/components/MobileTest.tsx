...
export function MobileTest({ churchId }: MobileTestProps) {
  ...
  useEffect(() => {
    const connect = () => {
      ...
      wsDisplayRef.current = ws;
      ws.onopen = () => setDisplayConnected(true);
      ws.onclose = () => { setDisplayConnected(false); setTimeout(connect, 2000); };
      ws.onmessage = (e) => {
    ...
    connect();
    return () => wsDisplayRef.current?.close();
  }, [churchId]);
  ...
  const flushBuffer = useCallback(() => {
    if (sendBufferRef.current.length === 0 || !wsAudioRef.current || wsAudioRef.current.readyState !== WebSocket.OPEN) return;
    ...
    wsAudioRef.current.send(JSON.stringify({ type: 'audio', audio: btoa(binary) }));
  }, []);
  ...
  const connectAudio = useCallback(() => {
    ...
    ws.onclose = () => {
      ...
      setTimeout(() => { if (shouldReconnectRef.current) connectAudio(); }, retryDelayRef.current);
  ...
  const stop = useCallback(() => {
    ...
    if (wsAudioRef.current) {
      if (wsAudioRef.current.readyState === WebSocket.OPEN) {
        wsAudioRef.current.send(JSON.stringify({ type: 'session.stop' }));
  ...
  // Live: translation feed + controls
  return (
    <div className="h-screen bg-gray-950 text-white flex flex-col overflow-hidden">
      ...
      <div className="flex-none flex items-center justify-between px-4 py-3 bg-gray-900 border-b border-gray-800">
        <div className="flex items-center gap-2">
          ...
          {!displayConnected && (
            <span className="text-xs text-yellow-500 ml-1">· display disconnected</span>
          )}
        </div>
        <button
          onClick={stop}
      ...
      >
        <div className="min-h-full flex flex-col justify-end px-5 pt-12 pb-6 gap-4">
          ...
          {segments.map((s) => (
            <div key={s.id} className="animate-fade-in space-y-0.5">
              <p className="text-xl font-semibold leading-snug">{s.english}</p>
